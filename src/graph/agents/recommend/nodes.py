...
判断标准（严格，宁可误判good也不误判poor）：
- good：只要结果中≥3家餐厅的品类与用户需求有合理关联
  例如：用户要火锅→搜到火锅/串串/烤肉/中餐厅 → good
  例如：用户要日料→搜到寿司/居酒屋/日式 → good
  例如：用户说"随便/附近"→任何餐厅 → good
  例如：用户否定某类(如不想吃火锅)→搜到非火锅餐厅 → good
- poor：仅当≥4家结果与需求完全不相关
  例如：用户要日料→全是西北面馆 → poor
  注意：以下情况不算poor：
  - "未体现用户偏好" → 不是poor
  - "结果不够多样" → 不是poor
  - "两家都是同类" → 不是poor
