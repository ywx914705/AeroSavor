...
用户需求：{user_query}
用户偏好：{user_preference}（偏好只是加分项，不是必要条件）
搜索结果（前5家）：
{poi_list}

判断标准：
- good：至少 3 家餐厅与用户需求的基本菜系/品类匹配
  例如：用户要火锅，结果有火锅店 → good（即使没体现偏好细节）
- good：结果数量充足（≥3家）且评分合理（平均≥3.5），即使不完美匹配也算good
- poor：仅当结果与需求的菜系严重不符（如用户要日料，结果全是中餐厅）
  注意：缺少偏好细节（如"未体现用户偏好"）不是poor的理由
